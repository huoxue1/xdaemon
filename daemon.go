...
type Daemon struct {
	ctx         context.Context
	LogFile     io.Writer //日志文件, 记录守护进程和子进程的标准输出和错误输出. 若为空则不记录
...
func Background(logFile io.Writer, isExit bool) (*exec.Cmd, error) {
	//判断子进程还是父进程
	runIdx++
	envIdx, err := strconv.Atoi(os.Getenv(ENV_NAME))
	if err != nil {
...
func NewDaemon(logFile io.Writer) *Daemon {
	return &Daemon{
		LogFile:     logFile,
		MaxCount:    0,
		MaxError:    3,
...
func startProc(args, env []string, logFile io.Writer) (*exec.Cmd, error) {
